...
export function DieStruckModel() {
  ...
  const faceGeometry = useMemo(() => {
    ...
    const radius = baseGeom.halfWidth / 0.9;
    for (let i = 0; i < pos.count; i++) {
      const x = pos.getX(i);
      const y = pos.getY(i);
      const r = Math.sqrt(x * x + y * y) / (radius * 0.9);
      const raised = 0.02 * (1 - r);
      pos.setZ(i, raised);
    }
  ...
  const faceMaterial = useMemo(() => {
    return new THREE.MeshStandardMaterial({
      map: mapTexture,
      metalness: hasUserDesign ? 0.15 : 0.75,
      roughness: hasUserDesign ? 0.45 : 0.25,
